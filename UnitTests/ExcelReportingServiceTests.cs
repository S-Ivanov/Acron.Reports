...
namespace ExcelReportingServiceLib.Tests
{
    [TestClass()]
    public class ExcelReportingServiceTests
    {
        [TestMethod()]
        public void ExcelReportingService_GetConfiguration()
        {
            IDictionary<string, string> configuration = ExcelReportingService.GetConfiguration("модуль=впвпвп ; класс = пвпв; пусто =");
            Assert.IsNotNull(configuration);
            Assert.AreEqual(2, configuration.Count);
        }

        [TestMethod()]
        public void ExcelReportingService_GetExcelReport()
        {
            var excelStream = (new ExcelReportingService()).GetExcelReport(SourceData,
                @"модуль = C:\Проекты\Excel\Acron.Reports\Grouping\bin\Debug\Grouping.dll; класс = Grouping.ReportGeneratorWithGraphics");
            if (excelStream != null)
            {

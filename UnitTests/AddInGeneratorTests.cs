...
{
    [TestClass()]
    public class AddInGeneratorTests
    {
        [TestMethod()]
        public void AddInGenerator_Generate()
        {
            var excelStream = (new AddInGenerator(@"C:\Проекты\Excel\Acron.Reports\Grouping\bin\Debug\Grouping.dll", "Grouping.ReportGeneratorWithGraphics")).Generate(SourceData, null);
            if (excelStream != null)
            {
